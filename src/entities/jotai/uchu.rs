...
impl Uchu {
    ...
    pub fn set_pos0_board_pc(&mut self, suji: i8, dan: i8, pc: Piece) {
        self.ky0.set_pc_by_sq(file_rank_to_sq(suji, dan), pc);
    }
    ...
    pub fn kaku_kifu(&self) -> String {
        ...
        for teme in 0..self.teme {
            let moveex = &self.kifu[teme];
            s.push_str(&format!("[{}] {}", teme, moveex));
        }
        s
    }
...
",
            ...
            ky.get_pc_by_sq(91),
            //                   ▲き,　                   ▲ぞ,                     ▲い,                     ▲ね,                     ▲う,                     ▲し,                     ▲ひ,
            ky.mg[Piece::R1 as usize],
            ky.mg[Piece::B1 as usize],
            ky.mg[Piece::G1 as usize],
            ky.mg[Piece::S1 as usize],
            ky.mg[Piece::N1 as usize],
            ky.mg[Piece::L1 as usize],
            ky.mg[Piece::P1 as usize],
            //                   ▽キ,                     ▽ゾ,                     ▽イ,                     ▽ネ,                     ▽ウ,                     ▽シ,                     ▽ヒ,
            ky.mg[Piece::R2 as usize],
            ky.mg[Piece::B2 as usize],
            ky.mg[Piece::G2 as usize],
            ky.mg[Piece::S2 as usize],
            ky.mg[Piece::N2 as usize],
...
",
    ...
    /// 入れた指し手の通り指すぜ☆（＾～＾）
    pub fn do_moveex(&mut self, moveex: &MoveEx) {
        // もう入っているかも知れないが、棋譜に入れる☆
        let phase = self.get_teban(&Person::Friend);
        let cap = self.ky.do_moveex(&phase, moveex);
        let teme = self.teme;
        self.kifu[teme] = *moveex;
    ...
    pub fn undo_moveex(&mut self) -> bool {
        if 0 < self.teme {
            // 棋譜から読取、手目も減る
            self.teme -= 1;
            let phase = self.get_teban(&Person::Friend);
            let moveex = &self.get_sasite();
            let cap = self.cap[self.teme];
            self.ky.undo_sasite(&phase, &moveex, &cap);
