...
/// 現局面での最善手を返すぜ☆（*＾～＾*）
///
pub fn think(mut uchu: &mut Uchu) -> MoveEx {
    ...
    // print_move_hashset( &some_moves_hashset );

    filtering_move_except_oute(&mut some_moves_hashset, &mut uchu);
    ...
    filtering_move_except_sennitite(&mut some_moves_hashset, &mut uchu);

    // 自殺手は省くぜ☆（＾～＾）
    filtering_move_except_suicide(&mut some_moves_hashset, uchu);
    ...
    if some_moves_hashset.len() == 0 {
        ...
        for ss_hash in some_moves_hashset {
            if i == index {
                //let result : MoveEx = moveex.clone();
                let moveex = MoveEx::from_hash(ss_hash);
                g_writeln(&format!("info solution:{}.", moveex));
                return moveex;
            }
            i += 1;
        }
