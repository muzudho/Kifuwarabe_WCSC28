...
pub fn test( line:&String, starts:&mut usize, len:usize, uchu:&mut Uchu ) {
    ...
    if 4<(len-*starts) && &line[*starts..*starts+5] == "mvsrc" {
        ...
        *starts += 1;
        ...
        g_writeln( &format!( "a・a・a = {}", intersect_point_on_plane(&a,&a,&a) ) );
        g_writeln( &format!( "a・a・b = {}", intersect_point_on_plane(&a,&a,&b) ) );
        g_writeln( &format!( "a・a・c = {}", intersect_point_on_plane(&a,&a,&c) ) );
        g_writeln( &format!( "a・b・a = {}", intersect_point_on_plane(&a,&b,&a) ) );
        g_writeln( &format!( "a・b・b = {}", intersect_point_on_plane(&a,&b,&b) ) );
        g_writeln( &format!( "a・b・c = {}", intersect_point_on_plane(&a,&b,&c) ) );
        g_writeln( &format!( "a・c・a = {}", intersect_point_on_plane(&a,&c,&a) ) );
        g_writeln( &format!( "a・c・b = {}", intersect_point_on_plane(&a,&c,&b) ) );
        g_writeln( &format!( "a・c・c = {}", intersect_point_on_plane(&a,&c,&c) ) );

        g_writeln( &format!( "b・a・a = {}", intersect_point_on_plane(&b,&a,&a) ) );
        g_writeln( &format!( "b・a・b = {}", intersect_point_on_plane(&b,&a,&b) ) );
        g_writeln( &format!( "b・a・c = {}", intersect_point_on_plane(&b,&a,&c) ) );
        g_writeln( &format!( "b・b・a = {}", intersect_point_on_plane(&b,&b,&a) ) );
        g_writeln( &format!( "b・b・b = {}", intersect_point_on_plane(&b,&b,&b) ) );
        g_writeln( &format!( "b・b・c = {}", intersect_point_on_plane(&b,&b,&c) ) );
        g_writeln( &format!( "b・c・a = {}", intersect_point_on_plane(&b,&c,&a) ) );
        g_writeln( &format!( "b・c・b = {}", intersect_point_on_plane(&b,&c,&b) ) );
        g_writeln( &format!( "b・c・c = {}", intersect_point_on_plane(&b,&c,&c) ) );

        g_writeln( &format!( "c・a・a = {}", intersect_point_on_plane(&c,&a,&a) ) );
        g_writeln( &format!( "c・a・b = {}", intersect_point_on_plane(&c,&a,&b) ) );
        g_writeln( &format!( "c・a・c = {}", intersect_point_on_plane(&c,&a,&c) ) );
        g_writeln( &format!( "c・b・a = {}", intersect_point_on_plane(&c,&b,&a) ) );
        g_writeln( &format!( "c・b・b = {}", intersect_point_on_plane(&c,&b,&b) ) );
        g_writeln( &format!( "c・b・c = {}", intersect_point_on_plane(&c,&b,&c) ) );
        g_writeln( &format!( "c・c・a = {}", intersect_point_on_plane(&c,&c,&a) ) );
        g_writeln( &format!( "c・c・b = {}", intersect_point_on_plane(&c,&c,&b) ) );
        g_writeln( &format!( "c・c・c = {}", intersect_point_on_plane(&c,&c,&c) ) );
        ...
        *starts += 1;
        ...
        g_writeln( &format!( "a　　a・a = {}", intersect_point_on_line_segment(&a,&a,&a) ) );
        g_writeln( &format!( "a　　a・b = {}", intersect_point_on_line_segment(&a,&a,&b) ) );
        g_writeln( &format!( "a　　a・c = {}", intersect_point_on_line_segment(&a,&a,&c) ) );
        g_writeln( &format!( "a　　a・d = {}", intersect_point_on_line_segment(&a,&a,&d) ) );
        g_writeln( &format!( "a　　b・a = {}", intersect_point_on_line_segment(&a,&b,&a) ) );
        g_writeln( &format!( "a　　b・b = {}", intersect_point_on_line_segment(&a,&b,&b) ) );
        g_writeln( &format!( "a　　b・c = {}", intersect_point_on_line_segment(&a,&b,&c) ) );
        g_writeln( &format!( "a　　b・d = {}", intersect_point_on_line_segment(&a,&b,&d) ) );
        g_writeln( &format!( "a　　c・a = {}", intersect_point_on_line_segment(&a,&c,&a) ) );
        g_writeln( &format!( "a　　c・b = {}", intersect_point_on_line_segment(&a,&c,&b) ) );
        g_writeln( &format!( "a　　c・c = {}", intersect_point_on_line_segment(&a,&c,&c) ) );
        g_writeln( &format!( "a　　c・d = {}", intersect_point_on_line_segment(&a,&c,&d) ) );
        g_writeln( &format!( "a　　d・a = {}", intersect_point_on_line_segment(&a,&d,&a) ) );
        g_writeln( &format!( "a　　d・b = {}", intersect_point_on_line_segment(&a,&d,&b) ) );
        g_writeln( &format!( "a　　d・c = {}", intersect_point_on_line_segment(&a,&d,&c) ) );
        g_writeln( &format!( "a　　d・d = {}", intersect_point_on_line_segment(&a,&d,&d) ) );

        g_writeln( &format!( "b　　a・a = {}", intersect_point_on_line_segment(&b,&a,&a) ) );
        g_writeln( &format!( "b　　a・b = {}", intersect_point_on_line_segment(&b,&a,&b) ) );
        g_writeln( &format!( "b　　a・c = {}", intersect_point_on_line_segment(&b,&a,&c) ) );
        g_writeln( &format!( "b　　a・d = {}", intersect_point_on_line_segment(&b,&a,&d) ) );
        g_writeln( &format!( "b　　b・a = {}", intersect_point_on_line_segment(&b,&b,&a) ) );
        g_writeln( &format!( "b　　b・b = {}", intersect_point_on_line_segment(&b,&b,&b) ) );
        g_writeln( &format!( "b　　b・c = {}", intersect_point_on_line_segment(&b,&b,&c) ) );
        g_writeln( &format!( "b　　b・d = {}", intersect_point_on_line_segment(&b,&b,&d) ) );
        g_writeln( &format!( "b　　c・a = {}", intersect_point_on_line_segment(&b,&c,&a) ) );
        g_writeln( &format!( "b　　c・b = {}", intersect_point_on_line_segment(&b,&c,&b) ) );
        g_writeln( &format!( "b　　c・c = {}", intersect_point_on_line_segment(&b,&c,&c) ) );
        g_writeln( &format!( "b　　c・d = {}", intersect_point_on_line_segment(&b,&c,&d) ) );
        g_writeln( &format!( "b　　d・a = {}", intersect_point_on_line_segment(&b,&d,&a) ) );
        g_writeln( &format!( "b　　d・b = {}", intersect_point_on_line_segment(&b,&d,&b) ) );
        g_writeln( &format!( "b　　d・c = {}", intersect_point_on_line_segment(&b,&d,&c) ) );
        g_writeln( &format!( "b　　d・d = {}", intersect_point_on_line_segment(&b,&d,&d) ) );

        g_writeln( &format!( "c　　a・a = {}", intersect_point_on_line_segment(&c,&a,&a) ) );
        g_writeln( &format!( "c　　a・b = {}", intersect_point_on_line_segment(&c,&a,&b) ) );
        g_writeln( &format!( "c　　a・c = {}", intersect_point_on_line_segment(&c,&a,&c) ) );
        g_writeln( &format!( "c　　a・d = {}", intersect_point_on_line_segment(&c,&a,&d) ) );
        g_writeln( &format!( "c　　b・a = {}", intersect_point_on_line_segment(&c,&b,&a) ) );
        g_writeln( &format!( "c　　b・b = {}", intersect_point_on_line_segment(&c,&b,&b) ) );
        g_writeln( &format!( "c　　b・c = {}", intersect_point_on_line_segment(&c,&b,&c) ) );
        g_writeln( &format!( "c　　b・d = {}", intersect_point_on_line_segment(&c,&b,&d) ) );
        g_writeln( &format!( "c　　c・a = {}", intersect_point_on_line_segment(&c,&c,&a) ) );
        g_writeln( &format!( "c　　c・b = {}", intersect_point_on_line_segment(&c,&c,&b) ) );
        g_writeln( &format!( "c　　c・c = {}", intersect_point_on_line_segment(&c,&c,&c) ) );
        g_writeln( &format!( "c　　c・d = {}", intersect_point_on_line_segment(&c,&c,&d) ) );
        g_writeln( &format!( "c　　d・a = {}", intersect_point_on_line_segment(&c,&d,&a) ) );
        g_writeln( &format!( "c　　d・b = {}", intersect_point_on_line_segment(&c,&d,&b) ) );
        g_writeln( &format!( "c　　d・c = {}", intersect_point_on_line_segment(&c,&d,&c) ) );
        g_writeln( &format!( "c　　d・d = {}", intersect_point_on_line_segment(&c,&d,&d) ) );
